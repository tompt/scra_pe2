...
def BASE64_ENCODE(TEXTO):
    # import base64
    #TEXTO = 'YmFzZTY0IGVuY29kZWQgc3RyaW5n'
...
def PROCURAR_PALAVRAS_E_LISTAR_CONTEUDOS_EM_PAGINAS_WEB(LISTA_DO_QUE_PROCURAMOS, URL, ID, MAQUINA, TOP_DE_FREQUENCIAS_QUE_QUEREMOS,MOSTRAR_TODOS_TERMOS):
    ...
    for PALAVRA in LISTA_DO_QUE_PROCURAMOS:
        ...
        if vezes:
            ...
            """Temos de codificar isto em base64 ou a BD nao aceita os caracteres manhosos do html e python"""
            #print(text)
            TEXTO=text.encode('UTF-8')
            CONTEUDO = BASE64_ENCODE(TEXTO)
            print("Normal: %s\n%s" % (len(CONTEUDO),CONTEUDO))
            CONTEUDO=b"%s".decode("utf-8") % CONTEUDO
            print(CONTEUDO)
            CONTEUDO="".join(map(str, CONTEUDO))
            print(CONTEUDO)
            ...
            """temporario"""
            CONTEUDO=TERMO_ENCONTRADO
            modulo_mysql.INSERIR("INSERT INTO resultados (url, estado, interesse,termos,operacao_origem,maquina,conteudo) VALUES ('%s', '%s', '%s','%s', '%s', '%s','%s');" % (URL,"feito","sim",TERMO_ENCONTRADO,ID,MAQUINA,CONTEUDO))
            #modulo_mysql.INSERIR("INSERT INTO resultados (url, estado, interesse,termos,operacao_origem,maquina,conteudo) VALUES ('%s', '%s', '%s','%s', '%s', '%s','%s');" % (URL,"feito","sim",TERMO_ENCONTRADO,ID,MAQUINA,CONTEUDO))
        else:
            pass
...
def REMOVER_LINKS_DUPLICADOS(LISTA_LINKS,ID):
    ...
    for LINK in LISTA_LINKS:
        ...
        if LINK not in REPETIDOS:
            ...
            modulo_mysql.INSERIR("INSERT INTO resultados (url, estado, interesse,termos,operacao_origem,maquina,conteudo) VALUES ('%s', '%s', '%s','%s', '%s', '%s', '%s');" % (LINK, "aguarda", "", "", ID, MAQUINA,""))
...
if not LISTA_SITES[0]:
    ...
    modulo_mysql.ACTUALIZAR("UPDATE resultados SET estado = '%s',maquina='%s',interesse='%s',conteudo='' WHERE id='%s';" % ('feito', MAQUINA, "nao", id_original))
